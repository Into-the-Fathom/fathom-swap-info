...
import { FACTORY_ADDRESS, BUNDLE_ID } from 'constants/index'

export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "fathom/fathomswap") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GLOBAL_CHART = gql`
  query fathomSwapDayDatas($startTime: Int!, $skip: Int!) {
    fathomSwapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
