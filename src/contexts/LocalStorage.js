...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(FATHOMSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(FATHOMSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
